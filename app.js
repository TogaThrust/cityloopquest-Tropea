...
function clqStorageKey(suffix) {
  const prefix = (window.CLQ_CITY && window.CLQ_CITY.storagePrefix) || 'tropea';
...
function getAudioHeardIndices() {
    try {
        const saved = localStorage.getItem('tropea_audio_heard_indices');
        if (saved) {
            const parsed = JSON.parse(saved);
            if (Array.isArray(parsed)) return parsed.map(Number).filter(n => !Number.isNaN(n));
        }
    } catch (_) { /* ignore */ }
    const legacy = localStorage.getItem('tropea_audio_clicked_index');
...
function getAudioCompletedIndices() {
    try {
        const saved = localStorage.getItem('tropea_audio_completed_indices');
        if (saved) {
            const parsed = JSON.parse(saved);
...
function markAudioCompletedForIndex(idx) {
    ...
    localStorage.setItem(
        'tropea_audio_completed_indices',
        JSON.stringify([...set])
    );
}
...
function markAudioHeardForIndex(idx) {
    ...
    const arr = [...set];
    try {
        localStorage.setItem('tropea_audio_heard_indices', JSON.stringify(arr));
        localStorage.setItem('tropea_audio_clicked_index', String(idx));
...
let completedQuizQuestions = (() => {
    const saved = localStorage.getItem("tropea_completedQuizQuestions");
...
const MONS_LAST_KNOWN_POSITION_KEY = 'tropea_lastKnownPosition';
...
function resetTourProgressForNewCircuit() {
  currentIndex = 0;
  localStorage.setItem('tropea_currentIndex', '0');
  localStorage.removeItem('tourCompleted');
  localStorage.removeItem('tropea_audio_heard_indices');
  localStorage.removeItem('tropea_audio_clicked_index');
  localStorage.removeItem('tropea_audio_completed_indices');
  localStorage.removeItem('tropea_quizEnabled');
  localStorage.removeItem('tropea_completedQuizQuestions');
  quizEnabled = false;
  quizResumeAwaitingAudio = false;
  quizPendingAdvanceCallback = null;
...
/** Variante fichier image Tropéa : espaces   underscores, casse conservée. */
...
function snapshotTourStateForExternalPage() {
    localStorage.setItem('tropea_currentIndex', String(currentIndex));
    localStorage.setItem('tropea_score', String(score));
    try {
...
function advanceToNextPoint() {
    ...
    if (currentIndex === filteredLocations.length - 2) {
        ...
        const visitedPoints = JSON.parse(localStorage.getItem('tropea_visitedPoints') || '[]');
        if (!visitedPoints.includes(currentIndex)) {
            visitedPoints.push(currentIndex);
            localStorage.setItem('tropea_visitedPoints', JSON.stringify(visitedPoints));
        }
        ...
        steps = [];
        currentStepIndex = 0;
        localStorage.setItem("tropea_currentIndex", currentIndex);
        localStorage.setItem("tropea_score", score);
...
function clearQuizCompletionFromIndex(fromIdx) {
  ...
  localStorage.setItem('tropea_completedQuizQuestions', JSON.stringify(completedQuizQuestions));
}
...
function updateQuizFooterButton() {
  ...
  const show = !guidanceMode && localStorage.getItem('tropea_quizEnabled') === 'false';
...
function setQuizEnabledRuntime(enabled) {
  quizEnabled = enabled;
  localStorage.setItem('tropea_quizEnabled', enabled ? 'true' : 'false');
  if (enabled) localStorage.removeItem('tropea_quizStopped');
  updateQuizFooterButton();
...
function stopQuizPermanently() {
  ...
  localStorage.setItem('tropea_quizStopped', 'true');
...
function showQuizPrompt(callback) {
  if (localStorage.getItem("tropea_quizEnabled") !== null) {
    callback(localStorage.getItem("tropea_quizEnabled") === "true");
    return;
  }
  const overlay = document.createElement('div');
  ...
  btnYes.style.cursor = 'pointer';
  btnYes.addEventListener('click', () => {
    localStorage.setItem("tropea_quizEnabled", "true");
    quizEnabled = true;
    completedQuizQuestions = {};
    localStorage.removeItem('tropea_completedQuizQuestions');
    localStorage.removeItem('tropea_quizStopped');
    document.body.removeChild(overlay);
    updateQuizFooterButton();
    callback(true);
  });
  ...
  btnNo.style.cursor = 'pointer';
  btnNo.addEventListener('click', () => {
    localStorage.setItem("tropea_quizEnabled", "false");
    quizEnabled = false;
    document.body.removeChild(overlay);
    updateQuizFooterButton();
    callback(false);
...
function initializeMainLogic() {
    ...
    const savedCurrentIndex = localStorage.getItem("tropea_currentIndex");
    const savedScore = localStorage.getItem("tropea_score");
    const savedQuizEnabled = freshCircuitStart
        ? null
        : localStorage.getItem("tropea_quizEnabled");
    const savedCompletedQuizQuestions = freshCircuitStart
        ? null
        : localStorage.getItem("tropea_completedQuizQuestions");
    ...
    if (forceTarget) {
        ...
        const visitedPoints = safeJsonParse(localStorage.getItem('tropea_visitedPoints'), []);
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    nextButton.addEventListener('click', () => {
        ...
        if (localStorage.getItem("tropea_quizEnabled") === null) {
            ...
            quizEnabled = localStorage.getItem("tropea_quizEnabled") === "true";
            if (quizEnabled) {
                showQuizForPointIndex(leavingIndex, advanceAfterQuiz, { force: true });
            } else {
                advanceAfterQuiz();
