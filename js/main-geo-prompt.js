...
(function () {
    ...
    function t(key, fb) {
        ...
        if (!tm || typeof tm.translate !== "function" || !tm.isLoaded) return fb;
    ...
    function setOverlayError(err) {
        ...
        if (code === 1) {
            text = t(
                "main_geo_activate_error_denied",
                "Accès refusé. Réglages > Confidentialité > Localisation > Safari > Lors de l'utilisation de l'app, puis réessayez."
            );
        } else if (code === 2) {
            text = t(
                "main_geo_activate_error_unavailable",
                "Position indisponible. Activez le GPS du téléphone et réessayez."
            );
        } else if (code === 3) {
            text = t(
                "main_geo_activate_error_timeout",
                "Délai dépassé. Sortez à l'extérieur si possible et réessayez."
            );
        } else {
            text = t(
                "main_geo_activate_error_generic",
        ...
        if (btn) {
            btn.disabled = false;
            btn.textContent = t("main_geo_activate_retry", "Réessayer");
        }
        showPrompt();
    }
    ...
    window.addEventListener("languageChanged", translateLabels);
